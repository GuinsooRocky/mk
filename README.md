...
### 已完成（v0.3）
- [x] 字典自动生长 — 启动后台 scan + mtime 缓存
- [x] 错例反馈 CLI — `MK --learn "错音" "正字"` + 幂等追加 + reload
- [x] 字典圈子 first-class — `polish.dictionary_domains` + `active_domains`
- [x] L1 single-pass + word boundary — 防 `bandwidth → bANDwidth`、`idea → IDEa`
- [x] L2 内置 ~500 高频英文词白名单 — 防 `AI → API` 类误纠（性能 < 20ms）
- [x] synth 排除英文 stop words — 防 `AND → and` 污染普通文本

### 下一步：让外部用户能用（"圈子选择"产品化）
- [ ] **预制领域包内置 .app**：科技 / 前端 / 后端 / AI / 产品 / PM / 设计 / 投资 等 ~8 个 starter pack
- [ ] **首启引导问卷**：「你做什么？」 → 自动启用对应圈子
- [ ] **错例反馈 GUI**：剪贴板监听 + 菜单栏「最近学了 N 个词」review 面板
- [ ] **iCloud sync learned 字典**：多设备共用，仅本人

### 仍 backlog
- [ ] LLM 后处理（C4）— 等找到合适端侧本地模型（不用 qwen3:0.6b；MK 哲学：尽量不上 LLM）
- [ ] OCR 屏幕术语自动 hint — 需用户授权屏幕录制
- [ ] 日语 ja-JP 支持 — 装系统语言包 + 改 locale 选择逻辑
- [ ] 真英文段落识别 — 当前 SA zh-CN 模型对真英文段落输出乱码（不在 MK 目标场景，但需要在 README 明确边界）
